...
const Dashboard = () => {
  ...
  const carregarDados = () => {
    ...
    const fetchPedidos = fetch('https://api-ecommerce-pi.onrender.com/api/pedidos')
  ...
  const handleSalvarProduto = async (e) => {
    ...
    try {
      const url = editandoId 
        ? `https://api-ecommerce-pi.onrender.com/api/produtos/${editandoId}` 
        : 'https://api-ecommerce-pi.onrender.com/api/produtos';
  ...
  const handleDeletar = async (id, nome) => {
    ...
    try {
      const resposta = await fetch(`https://api-ecommerce-pi.onrender.com/api/produtos/${id}`, { method: 'DELETE' });
  ...
  const handleAtualizarStatusPedido = async (id, novoStatus) => {
    try {
      const resposta = await fetch(`https://api-ecommerce-pi.onrender.com/api/pedidos/${id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
