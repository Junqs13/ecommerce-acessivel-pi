...
const Dashboard = () => {
  ...
  const carregarDados = () => {
    ...
    // Dispara as 4 buscas simultaneamente para o Render/Aiven
    ...
    Promise.all([fetchEstatisticas, fetchProdutos, fetchPedidos, fetchArtigos])
      .then(([estatisticas, listaProdutos, listaPedidos, listaArtigos]) => {
        // 1. Estatísticas
        if (estatisticas.geral) setDadosEstatisticos(estatisticas);
        else setErroApi(true);

        // 2. Produtos e Estoque Baixo
  ...
  const handleImagemUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => setNovoProduto({ ...novoProduto, imagem_url: reader.result });
  ...
  const handleSalvarProduto = async (e) => {
    ...
    try {
      ...
      if (resposta.ok) {
        toast.success(editandoId ? 'Produto atualizado!' : 'Produto adicionado!');
    ...
    } catch (error) { toast.error('Erro de conexão ao comunicar com a API.'); }
  };

  const handleEditar = (produto) => {
    setEditandoId(produto.id);
    setNovoProduto({ nome: produto.nome, descricao: produto.descricao, preco: produto.preco, categoria: produto.categoria, imagem_url: produto.imagem_url, quantidade: produto.quantidade !== undefined ? produto.quantidade : produto.estoque });
  ...
  const handleAtualizarStatusPedido = async (id, novoStatus) => {
    try {
      ...
      if (resposta.ok) { toast.success('Estado da encomenda atualizado!'); carregarDados(); } 
      else { toast.error('Erro ao atualizar o estado.'); }
    } catch (error) { toast.error('Erro de conexão com a API.'); }
  };
  ...
  const handleSalvarArtigo = async (e) => {
    e.preventDefault();
    try {
      const resposta = await fetch('https://api-ecommerce-oficial.onrender.com/api/artigos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (resposta.ok) {
        ...
        toast.error('Erro ao publicar artigo. Verifique se o título já não existe.');
      }
  ...
  if (erroApi) return <div style={{ textAlign: 'center', marginTop: '50px', color: 'red' }}><h2>⚠️ Erro de Comunicação com a API</h2><p>Verifique o banco de dados no Render/Aiven.</p></div>;
    ...
    <section aria-labelledby="titulo-dashboard" style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid var(--border)', paddingBottom: '15px', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setAbaAtiva('blog')} style={{ padding: '10px 20px', fontWeight: 'bold', border: '1px solid var(--border)', borderRadius: '5px', cursor: 'pointer', background: abaAtiva === 'blog' ? 'var(--primary)' : 'var(--bg-card)', color: abaAtiva === 'blog' ? '#fff' : 'var(--text-main)' }}>📝 Blog</button>
        </div>
      </div>

      {/* ================= ABA 1: ESTATÍSTICAS COM GRÁFICOS ================= */}
        ...
        <div>
          {produtosBaixoEstoque.length > 0 && (
            <div style={{ backgroundColor: '#fff3cd', color: '#856404', padding: '15px 20px', borderRadius: '8px', borderLeft: '5px solid #ffeeba', marginBottom: '20px' }}>
              <h3 style={{ margin: '0 0 10px 0' }}>⚠️ Produtos com Estoque Crítico</h3>
              <ul style={{ margin: 0, paddingLeft: '20px' }}>
                {produtosBaixoEstoque.map((p, index) => (
                  <li key={index}><strong>{p.nome}</strong> — Restam <span style={{ color: '#c0392b', fontWeight: 'bold' }}>{p.quantidade !== undefined ? p.quantidade : p.estoque}</span> unidades.</li>
                ))}
              </ul>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '30px' }}>
            <div style={{ flex: '1', minWidth: '200px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', borderLeft: '5px solid #004494', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ flex: '1', minWidth: '200px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', borderLeft: '5px solid #218838', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ margin: '0 0 10px 0', fontSize: '1rem', color: 'var(--text-muted)' }}>Volume em Armazém</h3>
              <p style={{ margin: 0, fontSize: '2rem', fontWeight: 'bold', color: '#218838' }}>{geral.total_itens || 0} peças</p>
            </div>
            <div style={{ flex: '1', minWidth: '200px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', borderLeft: '5px solid #e1b12c', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ margin: '0 0 10px 0', fontSize: '1rem', color: 'var(--text-muted)' }}>Patrimônio Projetado</h3>
          ...
          {/* Gráfico Recharts Restabelecido */}
          {porCategoria.length > 0 && (
            <div style={{ backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ marginTop: 0, marginBottom: '20px' }}>Distribuição do Catálogo por Categoria</h3>
              <div style={{ width: '100%', height: '300px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={porCategoria} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="categoria" stroke="var(--text-main)" />
                    <YAxis stroke="var(--text-main)" />
                    <Tooltip cursor={{fill: 'rgba(0,0,0,0.1)'}} />
                    <Legend />
                    <Bar dataKey="quantidade_modelos" name="Qtd. Modelos" fill="#004494" radius={[5, 5, 0, 0]} />
                    <Bar dataKey="total_pecas_estoque" name="Peças Físicas" fill="#e1b12c" radius={[5, 5, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      {abaAtiva === 'catalogo' && (
        <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1', minWidth: '300px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)', height: 'fit-content' }}>
            <h3 style={{ marginTop: 0, color: editandoId ? '#d35400' : 'inherit' }}>{editandoId ? `✏️ A editar: ${novoProduto.nome}` : '➕ Novo Instrumento'}</h3>
            <form onSubmit={handleSalvarProduto} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <input type="text" placeholder="Nome" required value={novoProduto.nome} onChange={(e) => setNovoProduto({...novoProduto, nome: e.target.value})} style={inputStyle} />
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <div style={{ flex: 1 }}><label style={labelStyle}>Estoque:</label><input type="number" required value={novoProduto.quantidade} onChange={(e) => setNovoProduto({...novoProduto, quantidade: e.target.value})} style={inputStyle} /></div>
              </div>
              <div>
                ...
                <select required value={novoProduto.categoria} onChange={(e) => setNovoProduto({...novoProduto, categoria: e.target.value})} style={{...inputStyle, width: '100%'}}>
                  <option value="Cordas">Cordas</option><option value="Teclas">Teclas</option><option value="Sopro">Sopro</option><option value="Percussão">Percussão</option><option value="Acessórios">Acessórios</option>
                </select>
              </div>
              <div style={{ border: '1px dashed var(--border)', padding: '10px', borderRadius: '6px' }}>
                <label style={labelStyle}>Imagem (Upload):</label>
                <input type="file" accept="image/*" onChange={handleImagemUpload} style={{ width: '100%', padding: '5px', marginBottom:'10px' }} />
                <label style={labelStyle}>Ou Link da Imagem (URL):</label>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button type="submit" className="btn btn-comprar" style={{ flex: 1, padding: '15px' }}>{editandoId ? '💾 Atualizar Produto' : '➕ Guardar Produto'}</button>
                {editandoId && (<button type="button" onClick={() => { setEditandoId(null); setNovoProduto({nome:'', descricao:'', preco:'', categoria:'Cordas', imagem_url:'', quantidade:''}); }} style={{ padding: '15px', background: '#ccc', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>Cancelar</button>)}
              </div>
            </form>
          </div>
          <div style={{ flex: '2', minWidth: '350px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead><tr style={{ borderBottom: '2px solid var(--border)' }}><th>Produto</th><th>Categoria</th><th>Preço</th><th>Estoque</th><th>Ações</th></tr></thead>
                <tbody>
                  {todosProdutos.map(p => (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '12px 0', fontWeight: '500' }}>{p.nome}</td>
                      ...
                      <td style={{ display: 'flex', gap: '5px', padding: '10px 0' }}>
                        <button onClick={() => handleEditar(p)} style={{ background: '#3498db', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '4px', cursor: 'pointer' }}>✏️</button>
                        <button onClick={() => handleDeletar(p.id, p.nome)} style={{ background: '#ff4d4d', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '4px', cursor: 'pointer' }}>🗑️</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {abaAtiva === 'vendas' && (
        <div style={{ backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead><tr style={{ borderBottom: '2px solid var(--border)' }}><th style={{ padding: '12px 10px' }}>ID</th><th>Cliente</th><th>Data</th><th>Total</th><th>Método</th><th>Estado Atual</th><th>Alterar Estado</th></tr></thead>
              <tbody>
                {pedidos.map(p => {
                  let corFundo = '#fff3cd'; let corTexto = '#856404';
                  if (p.status === 'Pago' || p.status === 'Enviado') { corFundo = '#cce5ff'; corTexto = '#004085'; }
                  if (p.status === 'Entregue') { corFundo = '#d4edda'; corTexto = '#155724'; }
                  if (p.status === 'Cancelado') { corFundo = '#f8d7da'; corTexto = '#721c24'; }
                  return (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td><span style={{ padding: '5px 10px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 'bold', backgroundColor: corFundo, color: corTexto }}>{p.status}</span></td>
                      <td>
                        <select value={p.status} onChange={(e) => handleAtualizarStatusPedido(p.id, e.target.value)} style={{ padding: '6px', borderRadius: '4px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-main)', cursor: 'pointer' }}>
                          <option value="Aguardando Pagamento">Aguardando Pagamento</option><option value="Pago">Pago</option><option value="Enviado">Enviado</option><option value="Entregue">Entregue</option><option value="Cancelado">Cancelado</option>
                        </select>
                      </td>
                    </tr>
                  );
                })}
                {pedidos.length === 0 && (<tr><td colSpan="7" style={{ textAlign: 'center', padding: '30px', color: 'var(--text-muted)' }}>Nenhuma venda registrada ainda.</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1', minWidth: '300px', backgroundColor: 'var(--bg-card)', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)', height: 'fit-content' }}>
            ...
            <form onSubmit={handleSalvarArtigo} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <button type="submit" className="btn btn-primary" style={{ padding: '15px', fontSize: '1.1rem' }}>🚀 Publicar Artigo</button>
            </form>
          </div>
              ...
              <tbody>
                {artigos.map(artigo => (
                  <tr key={artigo.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ display: 'flex', gap: '5px', padding: '10px 0' }}>
                      <button onClick={() => handleDeletarArtigo(artigo.id, artigo.titulo)} style={{ background: '#ff4d4d', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '4px', cursor: 'pointer' }}>🗑️ Apagar</button>
                    </td>
                  </tr>
                ))}
